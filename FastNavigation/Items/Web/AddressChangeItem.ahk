class AddressNavigationItem extends NavigationItem {
    ...
    ActivateItem(){
        Log(" Address change Item {1} -> {2}", [this.Letter, this.Description])

        if(not this.IsInBrowser()) {
            Log(" Address change : Run this command in browser window", [])
        ...
        if(StrLen(LocalUrl) > 200){
            Log(" Address change : Url is longer than 200. `n`r", LocalUrl)
        ...
        Log(" Address change : Could not match url to any Environment ", [])
